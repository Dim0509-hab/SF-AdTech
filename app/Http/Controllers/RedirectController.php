...
class RedirectController extends Controller {
    public function redirect(Request $request, $token){
        ...
        $sub = DB::table('offer_webmaster')
        ->where('offer_id',$click->offer_id)
        ->where('webmaster_id',$click->webmaster_id)->first();
        if (!$sub){ DB::table('rejections')
            ->insert(['offer_id'=>$click->offer_id,'webmaster_id'=>$click->webmaster_id,'ip'=>$request
            ->ip(),'referer'=>$request->headers->get('referer'),'attempted_at'=>now()]); abort(404);
        }
        $offer = $click->offer;
        $agreed = DB::table('offer_webmaster')
